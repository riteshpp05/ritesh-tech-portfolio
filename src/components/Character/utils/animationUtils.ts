...
const setAnimations = (gltf: GLTF) => {
  ...
  // Check if this is a rigged model (has bones) or a static bust/mesh
  ...
  const isRiggedModel = !!(spine || spine1 || neck || head);

  if (isRiggedModel) {
    // --- Rigged model bone adjustments (original Avaturn/Mixamo style) ---
    ...
    if (leftShoulder) leftShoulder.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0.15)));
    if (rightShoulder) rightShoulder.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, -0.15)));
    if (leftArm) leftArm.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(1.4, 0, 0.25)));
    if (rightArm) rightArm.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(1.4, 0, -0.25)));
    if (leftForeArm) leftForeArm.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0.2, 0.1)));
    if (rightForeArm) rightForeArm.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -0.2, -0.1)));
    if (leftHand) leftHand.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0.1, 0, 0)));
    if (rightHand) rightHand.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0.1, 0, 0)));
    if (leftUpLeg) leftUpLeg.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0.03)));
    if (rightUpLeg) rightUpLeg.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, -0.03)));
    if (neck) neck.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(-0.05, 0, 0)));

    // Play embedded animations if available
    if (gltf.animations && gltf.animations.length > 0) {
      const mainAction = mixer.clipAction(gltf.animations[0]);
      ...
      if (spine1) {
        ...
        const breathTrack = new THREE.QuaternionKeyframeTrack(
          `${spine1.name}.quaternion`, [0, 1.5, 3],
